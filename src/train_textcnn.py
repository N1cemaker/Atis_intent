...
def main():
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(best_state, "checkpoints/textcnn_best.pt")
    ...
    plt.tight_layout()
    plt.savefig("reports/figures/textcnn_loss_curve.png", dpi=300)
    plt.close()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("TextCNN Train/Validation Accuracy")
    plt.ylim(0.8, 1.0)   
    plt.legend()
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.tight_layout()
    plt.savefig("reports/figures/textcnn_train_val_acc_curve.png", dpi=300)
    plt.close()

    print("[Figure] Train/Val accuracy curve saved to reports/figures/textcnn_train_val_acc_curve.png")
